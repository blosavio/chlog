[:section#ideas
 ...
 [:p "The " [:code "changelog.edn"] " is the single, canonical source of information. All other representations (html/markdown, etc) are derived from that and are merely conveniences."]
 
 [:p#info "A human- and machine-readable " [:code "changelog.edn"] " will accompany each version. " [:code "changelog.edn"] " is tail-appended file constructed from all previous releases, possibly automatically-composed of per-version " [:code "changelog-v" [:em "N"] ".edn"] " files in a sub-directory."]
 
 [:h3 "A low threshold for breakage"]

 [:p "The Chlog experiment focuses on the changelog being the sole source of information on what will happen when switching versions. For that to work, the entries must accurately communicate whether a change is breaking. Not every change can be objectively categorized as either breaking or non-breaking (more on that in a moment). To have empathy for other people is tricky. If all changes are claimed as breaking, the concept loses its meaning and purpose. But if a supposedly safe change ends up breaking for someone else, trust is lost."]

 [:p "Within a changelog, seeing " [:code ":breaking? false"] " indicates that switching to that version will work as before with zero other changes (including changes in dependencies). Otherwise, the change is a " [:a {:href "#breaking"} [:em "breaking change"]] "."]
 ...
 [:p "Likewise, the follow kinds of changes are " [:strong "probably"] " non-breaking."]
